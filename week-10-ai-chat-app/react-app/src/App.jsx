...
function App() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      const res = await fetch(import.meta.env.VITE_AI_WORKER_URL, {
        method: "POST",
      ...
      if (!res.ok) {
        setError("Couldn't get response form AI");
        return;
      }

      const data = await res.json();

      const assistantContent = data.content || "Sorry, no response received.";
      setMessages([
        ...nextMessages,
  ...
  return (
    <div className="max-w-[960px] w-full md:w-[800px] mx-auto pt-8 px-5 pb-12 flex flex-col gap-5 font-sans">
      <header className="text-left flex justify-between items-end gap-5">
        <div>
          <h1 className="m-0 text-3xl md:text-4xl text-gray-900 dark:text-gray-100 font-medium tracking-tight">
            CodeWeekendAI Chat
          </h1>
          <p className="mt-2 max-w-[650px] text-gray-500 dark:text-gray-400 m-0">
            Ask your coding questions to the CodeWeekendAI.
          </p>
        </div>
      </header>
      ...
      <main className="flex flex-col gap-[18px] border border-gray-200 dark:border-gray-800 rounded-3xl bg-gray-50 dark:bg-[#1f2028] p-6 h-[80vh] min-h-[460px]">
        <div
          className="flex flex-col gap-[14px] flex-1 overflow-y-auto pr-1"
          ref={msgContainerRef}
        >
          {visibleMessages.map((message, index) => (
            <div
              key={index}
              className={`rounded-[20px] py-4 px-[18px] text-left border shadow-sm ${message.role === "user" ? "self-end bg-purple-100 dark:bg-purple-900/30 border-purple-200 dark:border-purple-800/50" : "self-start bg-white dark:bg-[#16171d] border-transparent dark:border-gray-800"}`}
            >
              <div className="text-xs mb-2.5 uppercase tracking-widest text-gray-500 dark:text-gray-400">
                {message.role === "user" ? "You" : "Assistant"}
              </div>
              <div className="whitespace-pre-wrap leading-[1.7] text-gray-900 dark:text-gray-100">
                {message.content}
              </div>
            </div>
          ))}
          ...
          {loading && (
            <div className="rounded-[20px] py-4 px-[18px] text-left border shadow-sm self-start bg-white dark:bg-[#16171d] border-transparent dark:border-gray-800 text-gray-400">
              ...
              <div className="whitespace-pre-wrap leading-[1.7] flex items-center gap-2 text-gray-400">
                <Loader2 className="animate-spin" /> AI is generating
                response...
              </div>
            </div>
          )}
        ...
        {error && (
          <div className="text-red-600 dark:text-red-400 text-sm text-left">
            {error}
          </div>
        )}
      </main>
